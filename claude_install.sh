...
# Experimental notes for claudex:
# - Official ChatGPT Codex endpoint observed in Codex binaries:
#   https://chatgpt.com/backend-api/codex
# - Auth token priority used by claudex fallback:
#   ~/.codex/auth.json -> tokens.id_token, then tokens.access_token, optional tokens.account_id header.
# - 2026-03-09: tool-call failures were largely caused by sending Anthropic `/v1/messages`
#   payloads directly to Responses-based upstreams. Current fix path in this repo:
#   - detect `wire_api = "responses"` (or ChatGPT codex fallback)
#   - rewrite `/v1/messages` -> `/v1/responses` for generic OpenAI-compatible upstreams
#   - rewrite `/v1/messages` -> `/responses` for `https://chatgpt.com/backend-api/codex`
